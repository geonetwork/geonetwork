...
1. gn4 webapp (setup to get postgresql and elastic)
...
2. run elastic
```
docker run --name elastic-gn5  -p 9200:9200 -p 9300:9300 -e "path.repo=/tmp" -e ES_JAVA_OPTS="-Xms750m -Xmx2g" -e "discovery.type=single-node" -e "xpack.security.enabled=false" -e "xpack.security.enrollment.enabled=false" docker.elastic.co/elasticsearch/elasticsearch:8.14.0
```
...
```
docker exec -it elastic-gn5 bash
cd /tmp
...
```
docker cp elastic-gn5:/tmp/es_backups.tar.gz .
```
...
```
docker kill elastic-gn5
docker rm elastic-gn5
docker run --name elastic-gn5  -p 9200:9200 -p 9300:9300 -e "path.repo=/tmp" -e ES_JAVA_OPTS="-Xms750m -Xmx2g" -e "discovery.type=single-node" -e "xpack.security.enabled=false" -e "xpack.security.enrollment.enabled=false" docker.elastic.co/elasticsearch/elasticsearch:8.14.0
```

copy old snapshot/backup to elastic container and un-tar it:
```
docker cp  es_backups.tar.gz elastic-gn5:/tmp/es_backups.tar.gz
docker exec   -w /tmp elastic-gn5 tar -xvzf es_backups.tar.gz
```
