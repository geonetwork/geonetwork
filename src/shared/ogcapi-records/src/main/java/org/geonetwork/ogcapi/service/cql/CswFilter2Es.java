...
public class CswFilter2Es extends AbstractFilterVisitor {
    ...
    private static final String TEMPLATE_NOT = " {\"bool\": {\n" + "            \"must_not\": [\n"
            ...
            + "            ]\n"
            + "          }}";
    private static final String TEMPLATE_AND =
            " {\"bool\": {\n" + "            \"must\": [\n" + "             %s\n" + "            ]\n" + "          }}";
    ...
    private static final String TEMPLATE_OR_WITH_FILTER = " \"bool\": {\n" + "            \"should\": [\n"
            ...
            + "            ]\n"
            + "          ,\"filter\":{\"query_string\":{\"query\":\"%s\"}}, \"minimum_should_match\" : 1}";
    private static final String TEMPLATE_MATCH =
            "{\"query_string\": {\n" + "        \"fields\": [\"%s\"],\n" + "        \"query\": \"%s\"\n" + "    }}";
    private static final String TEMPLATE_PROPERTY_IS_NOT =
    ...
    protected static String convertLikePattern(PropertyIsLike filter) {
        ...
        if (!filter.getWildCard().equals("*")) {
            final String wildcardRe = StringUtils.isNotEmpty(filter.getEscape())
                    ? Pattern.quote(filter.getEscape() + filter.getWildCard())
                    : filter.getWildCard();
            result = result.replaceAll(wildcardRe, "*");
        }
        if (!filter.getSingleChar().equals("?")) {
            final String singleCharRe = StringUtils.isNotEmpty(filter.getEscape())
    ...
    public String getFilter() {
        String condition = stack.isEmpty() ? null : stack.pop();
        return condition;
    }
    ...
    public Object visit(PropertyIsEqualTo filter, Object extraData) {
        ...
        final String filterEqualTo = String.format(
                TEMPLATE_MATCH, dataPropertyName, StringEscapeUtils.escapeJson(escapeLiteral(dataPropertyValue)));
    ...
    public Object visit(PropertyIsNotEqualTo filter, Object extraData) {
        ...
        filterPropertyIsNot = String.format(
                filterPropertyIsNot, dataPropertyName, StringEscapeUtils.escapeJson(escapeLiteral(dataPropertyValue)));
    ...
    public Object visit(BBOX filter, Object extraData) {
        ...
        final String coordsValue = String.format(Locale.US, "[[%f, %f], [%f, %f]]", x0, y1, x1, y0);
    ...
    private Object addGeomFilter(BinarySpatialOperator filter, String geoOperator, Object extraData) {
        ...
        final Expression geoExpression =
                filter.getExpression2() == null ? filter.getExpression1() : filter.getExpression2();
        ...
        try {
            ...
            log.error("Filter2ES", "Error parsing geospatial object", ex);
