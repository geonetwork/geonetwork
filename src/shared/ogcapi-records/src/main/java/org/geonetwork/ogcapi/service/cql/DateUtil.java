...
public class DateUtil {
    ...
    private static final Pattern gsYearMonth =
            Pattern.compile("([0-9]{4})-([0-1][0-9])(-([0-2][0-9]):([0-5][0-9])([A-Z]{0,1}))?");
    ...
    private static final Pattern htmlFormat = Pattern.compile(
    ...
    static {
        ...
        CATCH_ALL_DATE_TIME_FORMATTER = new DateTimeFormatterBuilder()
                .parseCaseInsensitive()
                .appendPattern(
                        "yyyy[[-]M][[-]d['T'H[:m[:s[.SSSSSSSSS][.SSSSSSSS][.SSSSSSS][.SSSSSS][.SSSSS][.SSSS][.SSS][.SS][.S]][XXX]]]]")
    ...
    public static String parseISODateTimes(String dateTimeString, String durationOrDateTimeString) {
        ...
        try {
            ...
            log.error(
                    "geonetwork.domain",
                    String.format("Error parsing ISO DateTimes '%s'. Error is: %s", dateTimeString, e.getMessage()),
                    e);
            return DEFAULT_DATE_TIME;
        }

        if (StringUtils.isBlank(durationOrDateTimeString)) return odt;
    ...
    public static ZonedDateTime parseBasicOrFullDateTime(String stringToParse) {
        ...
        if (stringToParse.length() == 8 && !stringToParse.startsWith("T")) {
            result = LocalDate.parse(stringToParse, DateTimeFormatter.BASIC_ISO_DATE)
            ...
            try {
                TemporalAccessor dt = CATCH_ALL_DATE_TIME_FORMATTER.parseBest(
    ...
    /**
     * Parses a time an return current date at that time. Both local and offset formats are supported, such as '10:15',
     ...
     */
    public static ZonedDateTime generateDate(
            int year, int month, int day, int second, int minute, int hour, String timezone) {
