...
import org.geonetwork.schemas.XSDSchemaDefinition;
import org.geonetwork.schemas.XSDTypeDefinition;
...
public class EditLib {
    ...
    private static final Map<String, Integer> htVersions = new ConcurrentHashMap<>();
    ...
    public static String addGmlNamespaceToFragment(String fragment) {
        if (fragment.contains("<gml:") && !fragment.contains("xmlns:gml=\"")) {
            log.debug("  Add missing GML namespace.");
    ...
    public void removeEditingInfo(Element md) {
        ...
        for (Element child : (List<Element>) new ArrayList(md.getChildren())) {
            if (!Edit.NAMESPACE.getPrefix().equals(child.getNamespacePrefix())) removeEditingInfo(child);
            else {
                child.detach();
            }
        }
    ...
    public Element findElement(Element md, String ref) {
        Element elem = md.getChild(RootChild.ELEMENT, Edit.NAMESPACE);

        if (elem != null && ref.equals(elem.getAttributeValue(Edit.Element.Attr.REF))) return md;
    ...
    public Element addElement(SchemaPlugin schema, Element el, String qname) throws Exception {
        log.debug("#### in addElement()");
        ...
        log.debug("#### - child namespace = {}", ns);
        log.debug("#### - child prefix = {}", prefix);
        log.debug("#### - parents first child = {}", el.getChildren().stream().findFirst());
        ...
        fillElement(schema, mdSugg, el, child);
    ...
    public void addFragment(String schema, Element targetElement, String qname, String fragment, boolean removeExisting)
        ...
        try {
            ...
            log.error("EditLib : Error parsing XML fragment {}.", fragment);
    ...
    private void addChildToParent(
        ...
        for (String singleType : type.getAlElements()) {
            ...
            log.debug("####   - child of type {}, list size = {}", singleType, existingForThisType.size());
            if (!qname.equals(singleType) || !removeExisting) {
                for (Element existingChild : existingForThisType) {
                    ...
                    log.debug("####		- add child {}", existingChild.toString());
                }
            }
            ...
            filterOnQname(existingAllType, "geonet:child").stream()
                    .filter(gnChild -> (gnChild.getAttributeValue("prefix") + ":" + gnChild.getAttributeValue("name"))
                            .equals(singleType))
                    .findFirst()
    ...
    public void addXMLFragments(String schema, Element md, Map<String, String> xmlInputs) throws Exception {
        ...
        for (Map.Entry<String, String> entry : xmlInputs.entrySet()) {
            ...
            if (el == null) {
                log.error(MSG_ELEMENT_NOT_FOUND_AT_REF + nodeRef);
                continue;
            }

            if (xmlSnippetAsString == null || xmlSnippetAsString.isEmpty()) {
                continue;
            }
            ...
            for (String fragment : fragments) {
                if (nodeName != null) {
                    ...
                    log.debug("Add XML fragment; {} to element with ref: {} replacing content.", fragment, nodeRef);
                    // clean before update
                    el.removeContent();
    ...
    /**
     * This does exactly the same thing as {@link #addElementOrFragmentFromXpath(Element, SchemaPlugin, String,
    ...
    public int addElementOrFragmentFromXpaths(
        ...
        for (Map.Entry<String, AddElemValue> entry : xmlAndXpathInputs.entrySet()) {
            ...
            final boolean isReplaceAllMode = propertyValue.getNodeValue() != null
                    && propertyValue.getNodeValue().getName().startsWith(SpecialUpdateTags.REPLACE_ALL);

            if (isReplaceAllMode) {
                // Remove all
                AddElemValue propertyValueToProcess = new AddElemValue(new Element("gn_delete"));

                addElementOrFragmentFromXpath(
                        metadataRecord, schema, xpathProperty, propertyValueToProcess, createXpathNodeIfNotExist);
                ...
                for (Element fragment : children) {
                    ...
                    boolean updated = addElementOrFragmentFromXpath(
                            metadataRecord, schema, xpathProperty, propertyValueToProcess, createXpathNodeIfNotExist);
                    if (updated) {
                        numUpdated++;
                    }
                ...
                boolean updated = addElementOrFragmentFromXpath(
                        metadataRecord, schema, xpathProperty, propertyValue, createXpathNodeIfNotExist);
                if (updated) {
                    numUpdated++;
                }
            }
     ...
     *
     * <p>To update the root element of the metadata use the xpath: "" (empty string)
     *
     * <p>
     *
     ...
     *
     * <p>Then the attribute of the element will be set instead of the text of the element.
     *
    ...
    public boolean addElementOrFragmentFromXpath(
        ...
        try {
            ...
            if (((nodeList.isEmpty() && createXpathNodeIfNotExist) || isCreateMode) && !isDeleteMode) {
                ...
                // xpath should be
                // gmd:identificationInfo/gmd:MD_DataIdentification/gmd:citation/gmd:CI_Citation/gmd:date
                ...
                if (indexOfRequiredPortion > 0) {
                    ...
                    if (selectResult != null) {
                        ...
                        if (elem == null) {
                            isUpdated = createAndAddFromXPath(metadataRecord, schema, requiredXPath, value);
                        } else if (elem instanceof Element element) {
                            isUpdated = createAndAddFromXPath(
                                    element, schema, xpathProperty.substring(indexOfRequiredPortion), value);
                        } else {
                            isUpdated = false;
                        }
                    }
                } else {
                    isUpdated = createAndAddFromXPath(metadataRecord, schema, xpathProperty, value);
                }
            } else {
                        ...
                        if (isDeleteMode) {
                            if (propNode instanceof Element) {
                                ...
                                if (parent != null) {
                                    ...
                                    if (SpecialUpdateTags.DELETE.equals(
                                ...
                                parent.removeAttribute(targetAttribute.getName(), targetAttribute.getNamespace());
                            }
                        } else {
                            ...
                            if (propNode instanceof Element && isValueXml) {
                                ...
                                doAddFragmentFromXpath(schema, value.getNodeValue(), (Element) propNode);
    ...
    private void doAddFragmentFromXpath(SchemaPlugin schema, Element fragment, Element propEl) throws Exception {
        ...
        if (newValue.getName().equals(SpecialUpdateTags.REPLACE)
            ...
            for (Object o : children) {
                if (o instanceof Element child) {
                    ...
                    final boolean childHasSameTypeAsTarget = child.getName().equals(propEl.getName())
                    ...
                    if (isReplace) {
                        ...
                        if (parent == null) {
                            log.error(String.format(
                                    " > adding fragment from XPath in element %s which has no parent. This"
                                            + " usually means that the element is not allowed in the XSD. Check this"
                                            + " element in the metadata record.",
                                    propEl.getName()));
                        } else {
                        ...
                        final Element newElement = addElement(schema, propEl, child.getQualifiedName());
                    ...
                    propEl.addContent(new Text(((Text) o).getText()));
                }
            }
     ...
     */
    private boolean createAndAddFromXPath(
            Element metadataRecord, SchemaPlugin schema, String xpathProperty, AddElemValue value) throws Exception {
        ...
        final XPathParser xpathParser =
                new XPathParser(new StringReader(cloneOfElementToAttachTo.getQualifiedName() + "/" + result.two()));
        ...
        while (currentToken != null
            ...
            if (currentToken.kind == XPathParserLocalConstants.TEXT
                ...
                if (depth > 0) {
                    ...
                    log.debug("Check if {} exists in {}", qualifiedName, currentNode.getName());
                    ...
                    if (nodeToCheck != null) {
                        ...
                        log.debug(" > add new node {} inserted in {}", qualifiedName, currentNode.getName());
        ...
        if (value.isXml()) {
            ...
            if (existingElement) {
                currentNode = addElement(schema, currentNode.getParentElement(), currentNode.getQualifiedName());
            }
            ...
            doAddFragmentFromXpath(schema, value.getNodeValue(), currentNode);
        } else {
            if (isAttribute) {
        ...
        @SuppressWarnings("unchecked")
        List<Attribute> attributeToAdd = new ArrayList<Attribute>(cloneOfElementToAttachTo.getAttributes());
    ...
    private Pair<Element, String> findLongestMatch(
            final Element metadataRecord,
        ...
        if (visited.get(nextIndex)) {
            return Pair.read(
                    bestMatch,
                    String.join("/", xpathPropertyParts.subList(indexOfBestMatch, xpathPropertyParts.size())));
        }
            ...
            for (int i = xpathPropertyParts.size() - 1; i > -1; i--) {
                ...
                if (result.result instanceof Element) {
                    return Pair.read(
                            ...
                            String.join("/", xpathPropertyParts.subList(i, xpathPropertyParts.size())));
                }
            }
            return Pair.read(
                    bestMatch,
    ...
    private SelectResult trySelectNode(
        ...
        try {
            ...
            log.warn("An illegal xpath was used to locate an element: {}", xpathProperty);
    ...
    private void fillElement(SchemaPlugin plugin, SchemaSuggestions sugg, Element parent, Element element)
            throws Exception {
        String parentName = parent.getQualifiedName();
        fillElement(plugin, sugg, parentName, element);
    }

    /**
     * @param plugin The metadata schema
     * @param sugg The suggestion configuration for the schema
     * @param parentName The name of the parent
     * @param element The element to fill
     */
    private void fillElement(SchemaPlugin plugin, SchemaSuggestions sugg, String parentName, Element element)
            throws Exception {
        ...
        log.debug("#### - isSimpleElement({}) = {}", elemName, isSimpleElement);
        ...
        XSDTypeDefinition type =
                xsdSchemaDefinition.getTypeInfo(xsdSchemaDefinition.getElementType(elemName, parentName));
        ...
        log.debug("#### - Type:");
        log.debug("####   - name = {}", type.getName());
        log.debug("####   - # attributes = {}", type.getAttributeCount());
        log.debug("####   - # elements = {}", type.getElementCount());
        log.debug("####   - # isOrType = {}", type.isOrType());
        log.debug("####   - type = {}", type);
        log.debug("#### - Has suggestion = {}", hasSuggestion);

        // -----------------------------------------------------------------------
        // --- handle attributes if mandatory or suggested
        //
        for (MetadataAttribute attr : type.getAlAttribs()) {
            log.debug("####   - {} attribute = {}", attr.name);
            log.debug("####     - required = {}", attr.required);
            log.debug("####     - suggested = {}", sugg.isSuggested(elemName, attr.name));
        ...
        if (!type.isOrType()) {
            for (int i = 0; i < type.getElementCount(); i++) {
                ...
                log.debug("####   - {} element = {}", i, childName);
                log.debug("####     - suggested = {}", childIsSuggested);
                log.debug("####     - is mandatory = {}", childIsMandatory);

                if ((childIsMandatory || childIsSuggested) && !childIsFiltered) {

                    XSDTypeDefinition elemType =
                            xsdSchemaDefinition.getTypeInfo(xsdSchemaDefinition.getElementType(childName, elemName));
                    ...
                    boolean childHasOneSuggestion = sugg.hasSuggestion(childName, elemType.getElementList())
                            && (CollectionUtils.intersection(elemType.getElementList(), childSuggestion)
                                            .size()
                                    == 1);
                    boolean childHasOnlyCharacterStringSuggestion =
                            childSuggestion.size() == 1 && childSuggestion.contains("gco:CharacterString");

                    log.debug("####     - is or type = {}", elemType.isOrType());
                    log.debug("####     - has suggestion = {}", childHasOneSuggestion);
                    log.debug("####     - elem type list = {}", elemType.getElementList());
                    log.debug("####     - suggested types list = {}", childSuggestion);
                    ...
                    if (xsdSchemaDefinition.isSimpleElement(elemName, childName)
                            || // eg. gco:Decimal
                            !elemType.isOrType()
                            || // eg. gmd:EX_Extent
                            (elemType.isOrType()
                        ...
                        String ns = getNamespace(childName, element, xsdSchemaDefinition);
                        ...
                        // Continue ....
                        fillElement(plugin, sugg, element, child);
                    } else {
                        // Logging some cases to avoid
                        if (log.isDebugEnabled()) {
                            if (elemType.isOrType() && isISOPlugin) {
                                if (elemType.getElementList().contains(isoPlugin.getBasicTypeCharacterStringName())
                                        && !childHasOneSuggestion) {
                                    log.debug(
                                            "####   - (INNER) Requested expansion of an OR element having"
                                                    + " gco:CharacterString substitute and no suggestion: {}",
                                            element.getName());
                                } else {
                                    log.debug(
                ...
                && type.getElementList().contains(isoPlugin.getBasicTypeCharacterStringName())
                && !hasSuggestion) {
            // expand element which have no suggestion
            // and have a gco:CharacterString substitute.
            // gco:CharacterString is the default.
            log.debug(
                    "####   - Requested expansion of an OR element having gco:CharacterString substitute and"
                            + " no suggestion: {}",
                    element.getName());
            ...
            log.debug("####   - WARNING : requested expansion of an OR element : {}", element.getName());
        }
    }
    ...
    public List<Element> searchChildren(String chName, Element md, String schema) throws Exception {
        ...
        XSDTypeDefinition containerType = mdSchema.getTypeInfo(chName);
        for (String elemName : containerType.getAlElements()) {
            log.debug("		-- Searching for child {}", elemName);
            List<Element> elems;
        ...
        if (hasContent) {
            ...
            if (!chName.contains(RootChild.CHOICE)) {
    ...
    public void expandTree(SchemaPlugin schema, Element md) throws Exception {
    ...
    public void expandElement(SchemaPlugin schema, Element md) throws Exception {
        log.debug("entering expandElement()");
        ...
        if (xsdSchemaDefinition.isSimpleElement(elemName, parentName)) {
            log.debug("is simple element");
            return;
        }
        XSDTypeDefinition type = xsdSchemaDefinition.getTypeInfo(elemType);
        log.debug("Type = {}", type);

        for (int i = 0; i < type.getElementCount(); i++) {
            String childQName = type.getElementAt(i);

            log.debug("- childName = {}", childQName);
            ...
            String childNS = getNamespace(childQName, md, xsdSchemaDefinition);

            log.debug("- name      = {}", childName);
            log.debug("- prefix    = {}", childPrefix);
            ...
            if (list.isEmpty() && !type.isOrType()) {
                ...
                Element newElem = createElement(
                        schema, elemName, childQName, childNS, type.getMinCardinAt(i), type.getMaxCardinAt(i));

                if (i == 0) insertFirst(md, newElem);
                else {
                    String prevQName = type.getElementAt(i - 1);
                    String prevName = getUnqualifiedName(prevQName);
                    String prevNS = getNamespace(prevQName, md, xsdSchemaDefinition);
                    insertLast(md, prevName, prevNS, newElem);
                }
            } else {
                log.debug("- {} children of this type already present", list.size());
                log.debug("- min cardinality = {}", type.getMinCardinAt(i));
                log.debug("- max cardinality = {}", type.getMaxCardinAt(i));
                ...
                for (int j = 0; j < list.size(); j++) {
                    ...
                    if (j > 0) listElem.setAttribute(new Attribute(Edit.Element.Attr.UP, Edit.Value.TRUE));
                ...
                if (list.size() < type.getMaxCardinAt(i))
                    insertLast(
                            ...
                            createElement(
                                    ...
                                    childQName,
                                    childNS,
                                    type.getMinCardinAt(i),
                                    type.getMaxCardinAt(i)));
            }
        }
        addAttribs(type, md, xsdSchemaDefinition);
    }
    ...
    public String getNamespace(String qname, Element md, XSDSchemaDefinition schema) {
    ...
    private String getNamespace(String qname, XSDSchemaDefinition schema) {
    ...
    private boolean equal(String childName, String childNS, Element el) {
        if (Edit.NAMESPACE.getURI().equals(el.getNamespaceURI())) {
            return RootChild.CHILD.equals(el.getName())
        ...
        } else return childName.equals(el.getName()) && childNS.equals(el.getNamespaceURI());
    }
    ...
    private boolean equal(Element el1, Element el2) {
        ...
        if (geonetNS.equals(elemNS1) && geonetNS.equals(elemNS2)) {
            if (!RootChild.CHILD.equals(el1.getName())) return false;
            if (!RootChild.CHILD.equals(el2.getName())) return false;
    ...
    public XSDTypeDefinition getType(XSDSchemaDefinition mds, Element elem) throws Exception {
    ...
    public Element createElement(String schema, Element child, Element parent) throws Exception {
        ...
        return createElement(
                schemaPlugin, parent.getQualifiedName(), child.getQualifiedName(), child.getNamespaceURI(), min, max);
    }

    /** Creates a new element for editing, adding all mandatory subtags. */
    private Element createElement(SchemaPlugin schema, String parent, String qname, String childNS, int min, int max)
            throws Exception {
        ...
        if (!xsdSchemaDefinition.isSimpleElement(qname, parent)) {
            String elemType = xsdSchemaDefinition.getElementType(qname, parent);
            ...
            if (type.isOrType()) {
                ...
                // In most of the cases, elements have gco:CharacterString as one of the possible
                // substitute.
                ...
                boolean isISOPlugin = schema instanceof ISOPlugin;
                ISOPlugin isoPlugin = isISOPlugin ? (ISOPlugin) schema : null;

                if (isISOPlugin
                        && type.getElementList().contains(isoPlugin.getBasicTypeCharacterStringName())
                        && !useSuggestion) {
                    log.debug("OR element having gco:CharacterString substitute and no suggestion: {}", qname);

                    Element basicTypeNode = isoPlugin.createBasicTypeCharacterString();
                    Element newElem = createElement(
                            schema, qname, basicTypeNode.getQualifiedName(), basicTypeNode.getNamespaceURI(), 1, 1);
                    child.addContent(newElem);
                } else {
                    action = "before"; // js adds new elements before this child
                    for (String chElem : type.getAlElements()) {
                        if (chElem.contains(RootChild.CHOICE)) {
                            List<String> chElems = recurseOnNestedChoices(xsdSchemaDefinition, chElem, parent);
    ...
    private List<String> recurseOnNestedChoices(XSDSchemaDefinition schema, String chElem, String parent)
            throws Exception {
        List<String> chElems = new ArrayList<>();
        String elemType = schema.getElementType(chElem, parent);
        XSDTypeDefinition type = schema.getTypeInfo(elemType);
        for (String subChElem : type.getAlElements()) {
            if (subChElem.contains(RootChild.CHOICE)) {
    ...
    private void addAttribs(XSDTypeDefinition type, Element md, XSDSchemaDefinition schema) {
        for (MetadataAttribute attr : type.getAlAttribs()) {

            Element attribute = new Element(RootChild.ATTRIBUTE, Edit.NAMESPACE);
            ...
            if (!present) attribute.setAttribute(new Attribute(Edit.Attribute.Attr.ADD, Edit.Value.TRUE));
            else if (!attr.required) attribute.setAttribute(new Attribute(Edit.Attribute.Attr.DEL, Edit.Value.TRUE));
    ...
    private String cleanRootFromXPath(String xpathProperty, Element metadataRecord) {
        ...
        if (xpathProperty.startsWith(metadataRecord.getQualifiedName() + "/")) {
            xpathProperty =
                    xpathProperty.substring(metadataRecord.getQualifiedName().length() + 1);
        }
    ...
    private interface XPathParserLocalConstants {
        ...
        int PARENT = 83;
        int DESCENDANT = 7;
        Set<Integer> ILLEGAL_KINDS = Set.of(PARENT, DESCENDANT);
    }
    ...
    private boolean edit_CHOICE_GROUP_SEQUENCE_in(String name) {
        return name.contains(RootChild.CHOICE) || name.contains(RootChild.GROUP) || name.contains(RootChild.SEQUENCE);
    }
}
