...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="metadata"
                as="node()"
                select="(/mdb:MD_Metadata|/mdb:MD_Metadata|/gmd:MD_Metadata|/gmd:MD_Metadata)"/>

    <!--
    TODO: include citation templates to generate bibtex citation for the dataset
    ...
    <xsl:function name="schema-org-fn:buildColumnId" as="xs:string">
        ...
        <xsl:value-of select="concat(
          $featureTypeId, '/',
          json-util:escapeForJson(($carrierOfCharacteristics/gfc:code/gco:CharacterString/text(), string($position))[1]))"/>
    </xsl:function>
  ...
  <xsl:template name="iso19115-3.2018toJsonLD" as="xs:string*">
    ...
    <xsl:for-each select="$record">
        ...
        <xsl:for-each select="$licences">
          <xsl:choose>
            <xsl:when test="(gcx:Anchor/@xlink:href[starts-with(., 'http')]
              ...
              "<xsl:value-of select="json-util:escapeForJson(
      ...
      -->
      <!-- TODO:  Migrate citation formatter
      ...
      "citeAs": "<xsl:value-of select="json-util:escapeForJson($bibTexCitation)"/>",
      -->
      ...
      <xsl:if test="$featureTypes">
        ...
        <xsl:for-each select="$featureTypes">
          <xsl:variable name="featureTypeId"
                        select="if (gfc:typeName/text() != '') then json-util:escapeForJson(gfc:typeName/text()) else 'featureType'"/>
          ...
          <xsl:variable name="fileObjectId"
                        select="$fileObjects[1]/json-util:escapeForJson((cit:linkage/*/text())[1])"
                        as="xs:string?"/>
          {
          "@type": "cr:RecordSet",
  ...
  <xsl:function name="schema-org-fn:toJsonLDLocalized" as="xs:string">
      ...
      <xsl:choose>
          ...
          <xsl:when test="gmd:PT_FreeText">
            ...
            <xsl:for-each select="gmd:PT_FreeText/gmd:textGroup">
              ...
              <xsl:value-of select="concat('&quot;@value&quot;: &quot;',
                                  json-util:escapeForJson(gmd:LocalisedCharacterString/text()),
                                  '&quot;')"/>,
          ...
          <xsl:otherwise>
              ...
              <xsl:value-of select="concat('&quot;',
                            json-util:escapeForJson($node/(gco:CharacterString|gcx:Anchor)),
                            '&quot;')"/>
          </xsl:otherwise>
      </xsl:choose>
    </xsl:function>
    ...
    <xsl:template name="related-record">
      <xsl:param name="uuid" as="xs:string"/>
        ...
        <xsl:variable name="associations"
                      as="node()?"/>
      <!-- TODO: XSL Java Fn Migration <xsl:variable name="associations"
