...
>
  ...
  <xsl:param name="attributes">
    <xsl:choose>
      <xsl:when test="//iso:rule[contains(@context,'@') or contains(@context,'attribute')]">true
      </xsl:when>
  ...
  <xsl:template name="generate-default-rules">
    ...
    <xsl:text>&#10;</xsl:text>
    <xsl:comment>This mode can be used to generate an ugly though full XPath for locators
    </xsl:comment>
    <xsl:text>&#10;</xsl:text>
  ...
  <xsl:template name="handle-pattern">
    <xsl:call-template name="process-pattern">
      ...
      <!-- Multilingual title -->
      <xsl:with-param name="titles" select="iso:title"/>
  ...
  <xsl:template match="iso:rule[not(@abstract='true')] ">
    ...
    <axsl:template match="{@context}"
                   priority="{1000 + count(following-sibling::*)}"
                   mode="M{count(../preceding-sibling::*)}">
      ...
      <xsl:otherwise>
        ...
        <xsl:choose>
          ...
          <xsl:otherwise>
            ...
            <xsl:value-of
              select='document("")//xhtml:p[@id=concat("sch-message-", $number)]/text()'/>
          </xsl:otherwise>
        </xsl:choose>
  ...
  <xhtml:div class="ErrorMessages">
    <!-- Where the error message contains dynamic information, the message has been split into an "a" and a "b" section.
      ...
      -->
    <xhtml:p id="sch-message-1">Schema error: Schematron elements in old and new namespaces found
    </xhtml:p>
    <xhtml:p id="sch-message-2">Schema error: in the queryBinding attribute, use 'xslt'</xhtml:p>
    <xhtml:p id="sch-message-3a">Fail: This implementation of ISO Schematron does not work with
    ...
    <xhtml:p id="sch-message-15">Schema error: The key element is not in the ISO Schematron
      namespace. Use the XSLT namespace.
    </xhtml:p>
    <xhtml:p id="sch-message-16">Markup Error: no name attribute in &lt;function></xhtml:p>
    <xhtml:p id="sch-message-17">Schema error: The function element is not in the ISO Schematron
      namespace. Use the XSLT namespace.
    </xhtml:p>
    <xhtml:p id="sch-message-18">Schema error: Empty href= attribute for include directive.
    </xhtml:p>
    ...
    <xhtml:p id="sch-message-21">Schema error: Use include to include fragments, not a whole
      schema
    </xhtml:p>
    <xhtml:p id="sch-message-22">Schema error: XSD schemas may only be imported if you are using the
      'xslt2' query language binding
    </xhtml:p>
    <xhtml:p id="sch-message-23">Schema error: The import-schema element is not available in the ISO
      Schematron namespace. Use the XSLT namespace.
    </xhtml:p>
    <xhtml:p id="sch-message-24">Warning: Variables should not be used with the "xpath" query
      language binding.
    </xhtml:p>
    <xhtml:p id="sch-message-25">Warning: Variables should not be used with the "xpath2" query
      language binding.
    </xhtml:p>
    ...
    <xhtml:p id="sch-message-28">Schema implementation error: This schema has abstract patterns, yet
      they are supposed to be preprocessed out already
    </xhtml:p>
    ...
    <xhtml:p id="sch-message-37a">Using the XSLT namespace with a prefix other than "xsl" in
      Schematron rules is not supported in this processor:
    </xhtml:p>
    <xhtml:p id="sch-message-37b"/>
    <xhtml:p id="sch-message-38a">Error: unrecognized element in ISO Schematron namespace: check
