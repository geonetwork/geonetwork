...
public final class IO {
    ...
    public static void copyDirectoryOrFile(@Nonnull final Path from, @Nonnull final Path to, boolean copyInto)
            throws IOException {
    ...
    public static Path toPath(URI uri) {
        try {
            ...
            if (uri.toString().startsWith("jar:")) {
                throw new IllegalStateException("The zip file references in URI: "
                        + uri
    ...
    private static URL createFsSpecificURL(URI uri, FileSystem fileSystem) throws MalformedURLException {
        if (fileSystem != null
                && uri.getScheme().equals(fileSystem.getPath(".").toUri().getScheme())) {
