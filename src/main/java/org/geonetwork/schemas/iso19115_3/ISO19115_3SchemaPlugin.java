...
import static org.geonetwork.schemas.iso19115_3.ISO19115_3Namespaces.XLINK;
...
@Component
@Slf4j(topic = "org.geonetwork.schemas.iso19139")
public class ISO19115_3SchemaPlugin extends SchemaPlugin
  ...
  public Set<AssociatedResource> getAssociatedParents(Element metadata) {
    ...
    if (StringUtils.isNotEmpty(parentAssociatedResourceType)) {
      associatedResources.forEach(
          parent -> parent.setAssociationType(parentAssociatedResourceType));

      try {
        String XPATH_FOR_PARENT_IN_AGGRGATIONINFO =
            "*//mri:associatedResource/*" + "[mri:associationType/*/@codeListValue = '%s']";
        final List<?> associatedParents =
            Xml.selectNodes(
                metadata,
  ...
  private AssociatedResource metadataRefAsAssociatedResource(Element sibling) {
    ...
    String associationType =
        sibling
            .getChild("associationType", ISO19115_3Namespaces.MRI)
  ...
  public List<Element> getTranslationForElement(Element element, String languageIdentifier) {
    final String path =
        ".//lan:LocalisedCharacterString" + "[@locale='#" + languageIdentifier + "']";
    try {
  ...
  public Element removeTranslationFromElement(Element element, List<String> langs)
    ...
    List<Element> nodesWithStrings =
        (List<Element>)
            Xml.selectNodes(element, "*//lan:PT_FreeText", Arrays.asList(ISO19115_3Namespaces.LAN));
    ...
    for (Element e : nodesWithStrings) {
      ...
      if (mainLangElement.size() == 1) {
        ...
        } else if (mainCharacterString.getAttribute("nilReason", ISO19115_3Namespaces.GCO)
            == null) {
      ...
      layers
          ...
          .forEach(
              uuid -> {
                ...
                coupledResource.setAttribute(
                    "nilReason", "synchronizedFromOGC", ISO19115_3Namespaces.GCO);
                ...
                Element operationMetadata =
                    new Element("SV_OperationMetadata", ISO19115_3Namespaces.SRV);
  ...
  public List<Extent> getExtents(Element metadataRecord) {
    ...
    ElementFilter bboxFinder =
        new ElementFilter("EX_GeographicBoundingBox", ISO19115_3Namespaces.GEX);
    @SuppressWarnings("unchecked")
    ...
    while (bboxes.hasNext()) {
      ...
      try {
        extents.add(
            new Extent(
                Double.valueOf(
                    box.getChild("westBoundLongitude", ISO19115_3Namespaces.GEX)
                        .getChild("Decimal", ISO19115_3Namespaces.GCO)
                        .getText()),
                Double.valueOf(
                    box.getChild("eastBoundLongitude", ISO19115_3Namespaces.GEX)
                        .getChild("Decimal", ISO19115_3Namespaces.GCO)
                        .getText()),
                Double.valueOf(
                    box.getChild("southBoundLatitude", ISO19115_3Namespaces.GEX)
                        .getChild("Decimal", ISO19115_3Namespaces.GCO)
                        .getText()),
                Double.valueOf(
                    box.getChild("northBoundLatitude", ISO19115_3Namespaces.GEX)
                        .getChild("Decimal", ISO19115_3Namespaces.GCO)
                        .getText())));
