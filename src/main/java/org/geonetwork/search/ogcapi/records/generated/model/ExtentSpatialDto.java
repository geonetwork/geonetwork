...
public class ExtentSpatialDto {
    ...
    @Valid
    private List<ExtentSpatialBboxInnerDto> bbox = new ArrayList<>();
    ...
    @Valid
    @Size(min = 1)
    @Schema(name = "bbox", description = "One or more bounding boxes that describe the spatial extent of the dataset. In the Core only a single bounding box is supported.  Extensions may support additional areas. The first bounding box describes the overall spatial extent of the data. All subsequent bounding boxes describe more precise bounding boxes, e.g., to identify clusters of data. Clients only interested in the overall spatial extent will only need to access the first bounding box in the array.", requiredMode = Schema.RequiredMode.NOT_REQUIRED)
    @JsonProperty("bbox")
    ...
    public String toString() {
        String sb = "class ExtentSpatialDto {\n" +
            "    bbox: " + toIndentedString(bbox) + "\n" +
