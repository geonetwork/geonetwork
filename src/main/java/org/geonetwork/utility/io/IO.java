...
package org.geonetwork.utility.io;
...
public final class IO {
  ...
  public static final DirectoryStream.Filter<Path> FILES_FILTER =
      new DirectoryStream.Filter<Path>() {
        @Override
        public boolean accept(Path entry) throws IOException {
  ...
  public static void deleteFile(Path file, boolean throwException, String loggerModule) {
    deleteFile(file, throwException);
  }

  public static void deleteFile(Path file, boolean throwException) {
    try {
      Files.delete(file);
    } catch (IOException e) {
      if (throwException) {
  ...
  public static void copyDirectoryOrFile(
      @Nonnull final Path from, @Nonnull final Path to, boolean copyInto) throws IOException {
   ...
   */
  public static void copyDirectoryOrFile(
      @Nonnull final Path from,
      @Nonnull final Path to,
      boolean copyInto,
      @Nullable final DirectoryStream.Filter<Path> filter)
      throws IOException {
    ...
    if (Files.isDirectory(from)) {
      Assert.isTrue(
          !Files.isRegularFile(actualTo),
          "cannot copy a directory to a file. From: " + from + " to " + actualTo);
      if (filter == null) {
  ...
  public static Path relativeFile(
      Path relativeToDirectory, Path fileOrDirectory, Path newRelativeTo) {
    return newRelativeTo.resolve(
        relativeToDirectory.relativize(fileOrDirectory).toString().replace('\\', '/'));
  }
  ...
  public static void deleteFileOrDirectory(Path path, final boolean ignoreErrors)
      throws IOException {
    ...
    try {
      ...
      if (uri.toString().startsWith("jar:")) {
        throw new IllegalStateException(
            "The zip file references in URI: "
                + uri
                + " has not been opened.  Before you can create this path you must first call"
                + " ZipUtil.openZipFS with the url to the zip file");
      }
  ...
  public static InputStream newInputStream(Path file) throws IOException {
    ...
    //        && ApplicationContextHolder.get().getBeansOfType(SystemInfo.class).size() > 0
  ...
  private static class CopyAllFiles extends SimpleFileVisitor<Path> {
    ...
    public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) throws IOException {
      Files.copy(
          file,
          relativeFile(from, file, actualTo),
  ...
  private static class CopyAcceptedFiles extends SimpleFileVisitor<Path> {
    ...
    public CopyAcceptedFiles(
        @Nonnull Path from, @Nonnull Path actualTo, @Nonnull DirectoryStream.Filter<Path> filter) {
      this.from = from;
      this.actualTo = actualTo;
      this.filter = filter;
    }
