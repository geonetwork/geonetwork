...
import org.geonetwork.schemas.constant.Geonet.Namespaces;
import org.geonetwork.schemas.exceptions.NoSchemaMatchesException;
import org.geonetwork.schemas.exceptions.SchemaMatchConflictException;
import org.geonetwork.schemas.utility.Pair;
import org.geonetwork.schemas.utility.Xml;
import org.geonetwork.utility.io.IO;
import org.geonetwork.utility.nio.NioPathAwareCatalogResolver;
import org.geonetwork.utility.resolver.ResolverWrapper;
import org.geonetwork.utility.resolver.SchemaPluginUrlRewrite;
...
public class SchemaManager {
  ...
  private static final String GEONET_SCHEMA_URI =
      "http://geonetwork-opensource.org/schemas/schema-ident";
  private static final Namespace GEONET_SCHEMA_PREFIX_NS =
      Namespace.getNamespace("gns", GEONET_SCHEMA_URI);
  ...
  public static SchemaPlugin getSchemaPlugin(String schemaIdentifier) {
    ...
    try {
      ...
      if (log.isDebugEnabled()) {
        log.debug(
            "No bean defined for the schema plugin '" + schemaIdentifier + "'. " + e.getMessage());
      }
    }
    return schemaPlugin;
  }
  ...
  public String autodetectSchema(Element md, String defaultSchema)
    ...
    try {
      ...
      if (schema == null) {
        ...
        if (schema != null && log.isDebugEnabled()) {
          log.debug(
              "  => Found schema " + schema + " using AUTODETECT(elements with value) examination");
        }
      }
      ...
      if (schema == null) {
        ...
        if (schema != null && log.isDebugEnabled()) {
          log.debug(
              "  => Found schema " + schema + " using AUTODETECT(elements with root) examination");
        }
      }
      ...
      if (schema == null && defaultSchema != null) {
        ...
        if (defaultSchemaOrDependencySchema != null) {
          log.warn(
              "  Autodetecting schema failed for "
                  + md.getName()
                  + " in namespace "
                  + md.getNamespace()
  ...
  private Element deleteSchemaFromPluginCatalog(String name, Element root) throws Exception {
    ...
    for (Content content : contents) {
      ...
      if (!uri.getName().equals("uri")
          || !uri.getNamespace().equals(Geonet.Namespaces.OASIS_CATALOG)) {
        if (log.isDebugEnabled()) {
  ...
  private int getHighestSchemaPluginCatalogId(String name, Element root) throws Exception {
    ...
    for (Content content : contents) {
      ...
      if (!uri.getName().equals("rewriteURI")
          || !uri.getNamespace().equals(Namespaces.OASIS_CATALOG)) {
        if (log.isDebugEnabled()) {
          log.debug("Skipping element " + uri.getQualifiedName() + ":" + uri.getNamespace());
        }
        continue;
   ...
   * @param root the list of elements from the schemaplugin-uri-catalog
   */
  private void createUriEntryInSchemaPluginCatalog(String name, int baseNrInt, Element root)
      throws Exception {
  ...
  private void processSchema(
      ApplicationContext applicationContext, Path schemasDir, Element schemaPluginCatRoot)
  ...
  private void extractMetadata(MetadataSchema mds, Path xmlIdFile)
      throws JDOMException, NoSuchFileException {
    Element root = Xml.loadFile(xmlIdFile);
    ...
    mds.setAppMinorVersionSupported(
        root.getChildText("appMinorVersionSupported", GEONET_SCHEMA_NS));
    mds.setAppMajorVersionSupported(
        root.getChildText("appMajorVersionSupported", GEONET_SCHEMA_NS));
   ...
   * record (updated on editing or in UFO).
   */
  private Map<String, MetadataSchemaOperationFilter> extractOperationFilters(Path xmlIdFile)
      throws Exception {
    Element root = Xml.loadFile(xmlIdFile);
  ...
  private List<Element> extractConvElements(Path xmlConvFile) throws Exception {
    if (!Files.exists(xmlConvFile)) {
      ...
      if (!root.getName().equals("conversions")) {
        throw new IllegalArgumentException(
            "Schema conversions file "
                + xmlConvFile
                + " is invalid, no <conversions> root element");
      }
      @SuppressWarnings("unchecked")
  ...
  private String compareElementsAndAttributes(Element md, int mode)
    ...
    if (log.isDebugEnabled()) {
      log.debug(
          "Schema autodetection starting on "
              + md.getName()
              + " (Namespace: "
              + md.getNamespace()
              + ") using mode: "
    ...
    if (matches.size() > 1) {
      throw new SchemaMatchConflictException(
   ...
   *     md
   */
  private boolean isMatchingElementInMetadata(
      Element needle, Element haystack, boolean checkValue) {
    boolean returnVal = false;
    @SuppressWarnings("unchecked")
    Iterator<Element> haystackIterator = haystack.getDescendants(new ElementFilter());
  ...
  private void copySchemaXSDsToWebApp(String name, Path schemaPluginDir) throws Exception {
    ...
    DirectoryStream.Filter<? super Path> xsdFilter =
    ...
    try (DirectoryStream<Path> schemaplugins =
        Files.newDirectoryStream(schemaPluginDir, xsdFilter)) {
      boolean missingXsdFiles = true;
      for (Path schemaplugin : schemaplugins) {
        IO.copyDirectoryOrFile(
            schemaplugin, webAppDirSchemaXSD.resolve(schemaplugin.getFileName()), false);
