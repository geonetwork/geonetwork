...
public class EditLib {
  ...
  public Element addElement(MetadataSchema mdSchema, Element el, String qname) throws Exception {
    ...
    log.debug("#### - parents first child = {}", el.getChildren().stream().findFirst().toString());
  ...
  public void addFragment(
      String schema, Element targetElement, String qname, String fragment, boolean removeExisting)
      throws Exception {
    ...
    try {
      ...
      throw new IllegalStateException(
          "EditLib : Error when loading XML fragment, " + e.getMessage());
    }
  ...
  private void addChildToParent(
    ...
    for (String singleType : type.getAlElements()) {
      ...
      log.debug(
          "####   - child of type {}, list size = {}", singleType, existingForThisType.size());
      ...
      filterOnQname(existingAllType, "geonet:child").stream()
          .filter(
              gnChild ->
                  (gnChild.getAttributeValue("prefix") + ":" + gnChild.getAttributeValue("name"))
                      .equals(singleType))
          .findFirst()
  ...
  public void addXMLFragments(String schema, Element md, Map<String, String> xmlInputs)
      throws Exception {
    ...
    for (Map.Entry<String, String> entry : xmlInputs.entrySet()) {
      ...
      for (String fragment : fragments) {
        if (nodeName != null) {
          ...
          log.debug(
              "Add XML fragment; {} to element with ref: {} replacing content.", fragment, nodeRef);
          // clean before update
          el.removeContent();
  ...
  public int addElementOrFragmentFromXpaths(
    ...
    for (Map.Entry<String, AddElemValue> entry : xmlAndXpathInputs.entrySet()) {
      ...
      final boolean isReplaceAllMode =
          propertyValue.getNodeValue() != null
  ...
  public boolean addElementOrFragmentFromXpath(
    ...
    try {
      ...
      final List<Object> nodeList =
          trySelectNode(metadataRecord, metadataSchema, xpathProperty, true).results;
  ...
  private void doAddFragmentFromXpath(
      MetadataSchema metadataSchema, Element fragment, Element propEl) throws Exception {
  ...
  private boolean createAndAddFromXPath(
    ...
    final XPathParser xpathParser =
        new XPathParser(
            new StringReader(cloneOfElementToAttachTo.getQualifiedName() + "/" + result.two()));
      ...
      if (currentToken.kind == XPathParserLocalConstants.TEXT
        ...
        if (depth > 0) {
          ...
          if (nodeToCheck != null) {
            ...
            } else if (metadataSchema.getElementValues(
                    qualifiedName, currentNode.getQualifiedName())
                != null) {
    ...
    if (value.isXml()) {
      ...
      if (existingElement) {
        currentNode =
            addElement(
                metadataSchema, currentNode.getParentElement(), currentNode.getQualifiedName());
      }
  ...
  private Pair<Element, String> findLongestMatch(
    ...
    if (visited.get(nextIndex)) {
      return Pair.read(
          bestMatch,
          String.join(
              "/", xpathPropertyParts.subList(indexOfBestMatch, xpathPropertyParts.size())));
    }
  ...
  private void fillElement(
      MetadataSchema schema, SchemaSuggestions sugg, Element parent, Element element)
      throws Exception {
   ...
   */
  private void fillElement(
      MetadataSchema schema, SchemaSuggestions sugg, String parentName, Element element)
      throws Exception {
    ...
    if (!type.isOrType()) {
      for (int i = 0; i < type.getElementCount(); i++) {
        ...
        if ((childIsMandatory || childIsSuggested) && !childIsFiltered) {
          ...
          boolean childHasOneSuggestion =
              sugg.hasSuggestion(childName, elemType.getElementList())
  ...
  private int enumerateTree(Element md, int ref, int parent) throws Exception {
    ...
    elem.setAttribute(
        new Attribute(
            Edit.Element.Attr.UUID, md.getQualifiedName() + "_" + UUID.randomUUID().toString()));
    md.addContent(elem);
        ...
        for (int j = 0; j < list.size(); j++) {
          ...
          listElem.setAttribute(
              new Attribute(
                  ...
                  listChild.getQualifiedName() + "_" + UUID.randomUUID().toString()));
  ...
  private Element createElement(
      MetadataSchema schema, String parent, String qname, String childNS, int min, int max)
      throws Exception {
    ...
    if (!schema.isSimpleElement(qname, parent)) {
      ...
      if (type.isOrType()) {
        ...
        if (isISOPlugin
            ...
            && !useSuggestion) {
          log.debug(
              "OR element having gco:CharacterString substitute and no suggestion: {}", qname);
  ...
  private List<String> recurseOnNestedChoices(MetadataSchema schema, String chElem, String parent)
      throws Exception {
  ...
  private void addValues(MetadataSchema schema, Element elem, String name, String parent)
      throws Exception {
  ...
  private boolean edit_CHOICE_GROUP_SEQUENCE_in(String name) {
    return name.contains(RootChild.CHOICE)
        || name.contains(RootChild.GROUP)
        || name.contains(RootChild.SEQUENCE);
  }
}
